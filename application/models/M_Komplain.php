...
class M_Komplain extends CI_Model {
  function dropdown_karyawan(){
      $query = $this->db->query('SELECT * FROM karyawan WHERE id_level = 1 or id_level = 3 ORDER BY id_level ASC ');
      return $query->result();
  }
  ...
  public function get_komplain_teknisi(){
    return $this->db->join('Barang', 'Barang.id_barang=Activity.id_barang')
                    ->join('Karyawan', 'Karyawan.id_karyawan=Activity.id_karyawan')
  ...
  public function get_history(){
    return $this->db->join('Activity', 'Activity.id_activity=History.id_activity')
                    ->order_by('History.id_activity','asc')
                    ->get('History')->result();
  }
  //** INSERT KOMPLAIN */
...
//** KONFIRMASI SELESAI */
  ...
  function karyawan(){
    $tampil = $this->db->query('SELECT * FROM karyawan');  
    return $tampil->result();
  }
  function activity(){
  ...
  public function email_teknisi(){
    $email_teknisi = $this->db->query('SELECT email from karyawan WHERE id_level = 2 or id_level = 3');
    return $email_teknisi->result_array();
  }
  public function ambil_email_karyawan($id_activity){
    $this->db->select('Karyawan.email')
             ->from('Karyawan')
             ->join('Activity', 'Activity.id_karyawan=Karyawan.id_karyawan')
             ->where('id_activity', $id_activity);
    $query = $this->db->get();
    $data = $query->result();
    return $data[0]->email;
